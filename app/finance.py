...
class FinanceCompiler:
    ...
    def total_customer_revenue(self) -> float:
        ...
        customer_orders = [
            ...
            if order.seller.__class__.__name__ == "Warehouse"
    ...
    def total_supplier_costs(self) -> float:
        ...
        supplier_orders = [
            order
            for order in self.orders
            if order.seller.__class__.__name__ == "Supplier"
            and order.status == "received"
        ]
    ...
    def display_orders(self, orders: List[Order]):
        ...
        print(
            f"{'Order ID':<10} {'Item':<15} {'Qty':<5} {'Total':<10} {'Buyer':<15} {'Seller':<15} {'Status':<10} {'Date'}"
        )
        print("-" * 105)
        for order in orders:
            print(
                f"{order.order_id:<10} {order.item.name:<15} {order.quantity:<5} £{order.total_price:<8.2f} "
