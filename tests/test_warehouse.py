...
class TestWarehouse(unittest.TestCase):
    ...
    def test_get_available_items(self):
        ...
        self.assertEqual(
            available_items[self.item], 20
        )  # 10 (initial) + 10 (added) = 20
    ...
    def test_place_order_success(self):
        ...
        self.assertEqual(
            len(self.customer.order_history), 1
        )  # The customer should have one order

    def test_remove_stock_item_not_found(self):
        """Test that removing stock raises ValueError when item is not found."""
        with self.assertRaises(ValueError) as context:
            self.warehouse.inventory.remove_stock(
                self.non_existent_item, 1
            )  # Use the non-existent item
    ...
    def test_remove_stock_not_enough_available(self):
        ...
        with self.assertRaises(ValueError) as context:
            self.warehouse.inventory.remove_stock(
                self.item, 20
            )  # Attempt to remove 20 items when only 10 are in stock
    ...
    def test_order_from_supplier_no_items_available(self):
        ...
        with self.assertRaises(ValueError):
            self.warehouse.order_from_supplier(
                new_supplier, self.cloned_item, 5
            )  # No items available
    ...
    def test_mark_order_as_received_order_not_found(self):
        ...
        result = self.warehouse.mark_order_as_received(
            order_id=9999
